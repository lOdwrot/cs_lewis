...
async function setPublicPermissions(strapi) {
    ...
    const contentTypes = [
        ...collectionTypes.flatMap((ct) => ["find", "findOne"].map((a) => `${ct}.${a}`)),
...
async function publishDoc(strapi, uid, documentId) {
...
async function seedIfEmpty(strapi) {
    ...
    const stepImageByTitle = {
        "Sehnsucht: Ból Radości": pokeSign === null || pokeSign === void 0 ? void 0 : pokeSign.id,
    };
    const journeyImageBySlug = {
        "wizja-mitopoetyczna": pikaArt === null || pikaArt === void 0 ? void 0 : pikaArt.id,
    };
    ...
    const gateBackgroundBySlug = {
        wyobraznia: gateBg1 === null || gateBg1 === void 0 ? void 0 : gateBg1.id,
        rozum: gateBg2 === null || gateBg2 === void 0 ? void 0 : gateBg2.id,
        wiara: gateBg3 === null || gateBg3 === void 0 ? void 0 : gateBg3.id,
    };
    // ── Books ────────────────────────────────────────────────────────────────
    const books = [
        {
            title: "Kroniki Narnii",
            description: "Fundamentalna seria siedmiu powieści fantasy, przenosząca czytelników do krainy rozmawiających zwierząt i epickich zmagań dobra ze złem.",
            redirectUrl: "https://www.amazon.com/s?k=chronicles+of+narnia+cs+lewis",
        },
        {
            title: "Chrześcijaństwo po prostu",
            description: "Wywodzące się z audycji radiowych BBC, to klasyczne dzieło bada wspólny grunt, na którym stoi każdy wyznający wiarę chrześcijańską.",
            redirectUrl: "https://www.amazon.com/s?k=mere+christianity+cs+lewis",
        },
        {
            title: "Listy Starszego Diabła",
            description: "Satyryczne arcydzieło opowiedziane przez listy starszego demona do swojego bratanka, stanowiące przenikliwe spojrzenie na ludzką naturę.",
            redirectUrl: "https://www.amazon.com/s?k=screwtape+letters+cs+lewis",
        },
        {
            title: "Wielki Rozwód",
            description: "Teologiczna wizja senna o podróży autobusem z Piekła do Nieba, badająca wybory prowadzące do ostatecznego oddzielenia lub zbawienia.",
            redirectUrl: "https://www.amazon.com/s?k=great+divorce+cs+lewis",
        },
        {
            title: "Cuda",
            description: "Filozoficzna obrona możliwości Bożej interwencji, argumentująca, że świat przyrody nie jest zamkniętym systemem.",
...
Późniejszy Lewis rozumiał coś, co wcześniejszy Lewis mógł jedynie uzasadniać: że wiary nie utrzymuje się samą logiką, lecz codziennym wyborem zaufania Bogu, który wydaje się milczeć — nawet gdy megafon zamilkł.`,
    ...
    for (const gData of gatesData) {
        ...
        if (backgroundId)
            data.backgroundImage = backgroundId;
...
async function seedHomePageIfMissing(strapi) {
    ...
    const gateRelations = gates.map((g) => ({
        documentId: g.documentId,
    }));
    const newsItems = await strapi.db
        .query("api::news.news")
        .findMany({ orderBy: { createdAt: "asc" } });
    const newsRelations = newsItems.map((n) => ({
    ...
    const data = {
        ...
        subtitle: "Eseje, analizy i komentarze ukazujące myśl C.S. Lewisa jako spójną całość, w której wyobraźnia, rozum i wiara wzajemnie się przenikają",
        content: "Lewis nie oddzielał opowieści od argumentu ani argumentu od wiary. Uważał, że wyobraźnia przygotowuje grunt dla rozumu, rozum domaga się prawdy, a wiara nie niszczy żadnego z nich. Ta strona proponuje lekturę Lewisa właśnie w tym duchu — poprzez teksty, które prowadzą różnymi drogami ku tym samym pytaniom.",
...
async function seedTermsIfEmpty(strapi) {
...
async function uploadImageOnce(strapi, { filename, alternativeText }) {
    const existing = await strapi.db
        .query("plugin::upload.file")
        .findOne({ where: { name: filename } });
    if (existing)
        return existing;
    const filePath = node_path_1.default.join(strapi.dirs.static.public, filename);
...
async function seedGatePageIfMissing(strapi) {
    const existing = await strapi.db
...
async function seedJourneysPageIfMissing(strapi) {
    const existing = await strapi.db
    ...
    const data = {
        ...
        seoDescription: "Przeglądaj wszystkie podróże z myślą C.S. Lewisa — filtruj według poziomu trudności i znajdź swoją drogę.",
        heroLabel: "Odkryj",
        heroDescription: "Przeglądaj wszystkie podróże przez świat myśli C.S. Lewisa",
...
async function seedBooksPageIfMissing(strapi) {
    const existing = await strapi.db
    ...
    const data = {
        ...
        seoDescription: "Odkryj najważniejsze książki C.S. Lewisa — od Narni po apologetykę i filozofię chrześcijańską.",
        heroLabel: "Starannie Dobrana Kolekcja",
        heroDescription: "„Czytamy, by wiedzieć, że nie jesteśmy sami.” Odkryj najważniejsze dzieła C.S. Lewisa, gdzie rozum spotyka wyobraźnię w dążeniu do wiecznych prawd.",
...
async function seedEncyclopediaPageIfMissing(strapi) {
    const existing = await strapi.db
    ...
    const data = {
        title: "Encyklopedia",
        description: "Słownik pojęć, postaci i miejsc kluczowych dla zrozumienia myśli i twórczości C.S. Lewisa. Przeszukuj alfabetycznie lub korzystaj z wyszukiwarki, by szybko znaleźć interesujące hasło.",
    };
    if (file === null || file === void 0 ? void 0 : file.id) {
        data.backgroundImage = file.id;
    }
    const doc = await strapi
        ...
        .create({ data: data });
    await publishDoc(strapi, "api::encyclopedia-page.encyclopedia-page", doc.documentId);
...
async function seedNewsIfEmpty(strapi) {
...
const ARTICLES_SEED = [
    {
        ...
        description: "Analiza krytyki naturalizmu, którą Lewis rozwinął w trzecim rozdziale Cudów — być może najmocniejsza linia jego apologetyki.",
...
Współcześni filozofowie — Alvin Plantinga w *Where the Conflict Really Lies*, Thomas Nagel w *Mind and Cosmos* — kontynuują linię argumentacji Lewisa pod nazwą **Evolutionary Argument Against Naturalism**. To, co Lewis postawił w 1947 roku, dziś jest jednym z najbardziej żywych sporów w filozofii umysłu.`,
    },
    {
        ...
        description: "Drogowskaz, nie cel — Lewisowska tęsknota jako klucz do jego biografii i apologetyki.",
...
> „Jeśli odkrywam w sobie pragnienie, którego żadne doświadczenie tego świata nie może zaspokoić, najbardziej prawdopodobnym wyjaśnieniem jest to, że zostałem stworzony dla innego świata."`,
    },
    {
        ...
        description: "Wtorkowe poranki w Eagle and Child — jak nieformalne spotkania kilku oksfordzkich akademików dały światu Narnię i Śródziemie.",
...
Krąg zaczął się rozluźniać po śmierci Williamsa w 1945 i całkowicie rozpadł się około 1949 — ale jego owoce ukształtowały dwudziestowieczną literaturę fantastyczną.`,
    },
    {
        ...
        description: "Trzydzieści lat zażyłości, wzajemnego wpływu i powolnego oddalenia — co naprawdę dzieliło dwóch największych Inklingów.",
...
Mimo wszystko, gdy Lewis zmarł 22 listopada 1963 (w dniu zabójstwa Kennedy'ego), Tolkien napisał do córki: „Dotąd czułem normalne uczucia człowieka w moim wieku — jak stare drzewo, które po kolei traci liście. To czuję teraz jak cios siekiery, który ścina korzenie."`,
    },
    {
        ...
        description: "Najbardziej znany — i najbardziej krytykowany — argument apologetyczny Lewisa, w pełnej formie i z odpowiedziami na zarzuty.",
...
Mimo krytyki, trylemat pozostaje jednym z najczęściej cytowanych argumentów apologetycznych XX wieku. Filozofowie tacy jak William Lane Craig i Stephen Davis bronią go w zaktualizowanych formach, podczas gdy historycy biblijni jak N. T. Wright dostarczają mu współczesnego oparcia w badaniach nad świadomością mesjańską Jezusa.`,
    },
    {
        ...
        description: 'Jak rzymski filozof z VI wieku ukształtował obraz uporządkowanego kosmosu, który Lewis odtworzył w „Odrzuconym obrazie".',
...
Lewis nie zaprzecza nauce nowożytnej. Twierdzi jedynie, że średniowieczny obraz świata jest *jednocześnie* odrzucony i niezastąpiony — odpowiadał na pytania, których współczesna kosmologia nie próbuje już zadawać.`,
    },
    {
        ...
        description: "Satyra teologiczna z 1942 roku — jak Lewis wykorzystał formę listów piekielnego biurokraty do zdiagnozowania duchowych chorób nowoczesności.",
...
async function seedArticlesIfEmpty(strapi) {
...
async function seedLibraryPageIfMissing(strapi) {
    const existing = await strapi.db
    ...
    const data = {
        title: "Biblioteka",
        description: "Zbiór esejów, analiz i komentarzy poświęconych życiu i myśli C.S. Lewisa. Przeszukuj według tytułu lub przeglądaj listę alfabetycznie — każdy artykuł stanowi samodzielną lekturę.",
    };
    if (file === null || file === void 0 ? void 0 : file.id) {
        data.backgroundImage = file.id;
    }
...
async function seedBiographyPageIfMissing(strapi) {
    const existing = await strapi.db
        .query("api::biography-page.biography-page")
        .findOne({});
    if (existing)
        return;
    ...
    const sharedImage = (eventImage === null || eventImage === void 0 ? void 0 : eventImage.id) ? { image: eventImage.id } : {};
    const events = [
        {
            year: "1898",
    ...
    const data = {
        title: "Życiorys",
        description: "Linia czasu życia C.S. Lewisa — od dzieciństwa w Belfaście, przez doświadczenie wojny i lata w Oksfordzie, po nawrócenie i dzieła, które zdefiniowały jego epokę.",
        events,
    };
    if (backgroundFile === null || backgroundFile === void 0 ? void 0 : backgroundFile.id) {
        data.backgroundImage = backgroundFile.id;
    }
    const doc = (await strapi
        .documents("api::biography-page.biography-page")
        .create({ data: data }));
    await publishDoc(strapi, "api::biography-page.biography-page", doc.documentId);
